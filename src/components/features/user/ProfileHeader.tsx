...
export default function ProfileHeader({
  ...
  // Fallback seguro para handle
  const safeHandle = normalizeHandle(member?.handle) || suggestHandle(member);
    ...
    <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full">
        ...
        <div className="flex-1 flex flex-col text-white space-y-1">
          ...
          <p
            className="text-gray-400 text-sm whitespace-pre-wrap break-words max-w-[40ch] md:max-w-[60ch]"
            title={member?.bio || ""}
          >
            {member?.bio || t("no_bio")}
          </p>
