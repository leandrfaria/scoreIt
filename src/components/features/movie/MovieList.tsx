...
export function MovieList() {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => setDebouncedSearchTerm(searchTerm), 1000);
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center gap-2 mt-10 mb-20 text-white">
        ...
        <button onClick={() => handleArrowClick("prev")} className="text-darkgreen text-3xl hover:scale-110 transition">
          {"<"}
        </button>
        <input
          type="text"
        ...
        />
        <button onClick={() => handleArrowClick("next")} className="text-darkgreen text-3xl hover:scale-110 transition">
          {">"}
        </button>
      </div>
    </>
