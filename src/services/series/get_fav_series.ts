...
/**
 * Mantive a assinatura original (token, id) para compatibilidade,
 * mas usamos `apiFetch` com `{ auth: true }`.
 */
export const fetchFavouriteSeries = async (_token: string, memberId: string): Promise<Series[]> => {
  try {
